                ...
                <div class="userccount">
                    <div class="userbtns">
                        <ul class="nav nav-tabs nav-tabs2">
                            ...
                            <li class="{{($c_or_e == 'recruiter')? 'active':''}}"><a data-toggle="tab" href="#recruiter" aria-expanded="false">{{__('Recruiter')}}</a></li>
                        </ul>
                    </div>
                    <div class="tab-content">
                        <div id="candidate" class="formpanel tab-pane fade {{($c_or_e == 'candidate')? 'active in':''}}">
                            ...
                            <!-- sign up form -->
                            <div class="newuser"><i class="fa fa-user" aria-hidden="true"></i> {{__('New User')}}? <a href="{{route('register')}}">{{__('Register Here')}}</a>
                            </div>
                            <div class="newuser">
                        ...
                        <div id="recruiter" class="formpanel tab-pane fade {{($c_or_e == 'recruiter')? 'active in':''}}">
                            <div class="socialLogin">
                                <h5>{{__('Login')}}</h5>
                                <!-- <h5>{{__('Login with Social')}}</h5>
                                        <a href="{{ url('login/employer/facebook')}}" class="fb"><i class="fa fa-facebook" aria-hidden="true"></i></a> <a href="{{ url('login/employer/twitter')}}" class="tw"><i class="fa fa-twitter" aria-hidden="true"></i></a>-->
                            </div>
                            <form class="form-horizontal" method="POST" action="{{ route('recruiter.login') }}">
                                {{ csrf_field() }}
                                <input type="hidden" name="candidate_or_employer" value="recruiter" />
                                <div class="formpanel">
                                    <div class="formrow{{ $errors->has('email') ? ' has-error' : '' }}">
                                        <input id="email" type="email" class="form-control" name="email" value="{{ old('email') }}" required autofocus placeholder="{{__('Email Address')}}">
                                        @if ($errors->has('email'))
