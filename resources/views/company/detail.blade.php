            ...
            <div class="jobButtons">
                @if(!(Auth::guard('company')->user() || Auth::guard('recruiter')->user()))
                    @if(Auth::check() && Auth::user()->isFavouriteCompany($company->slug)) 
                        <a href="{{route('remove.from.favourite.company', $company->slug)}}" class="btn report">
                            <i class="fa fa-remove" aria-hidden="true"></i> {{__('Remove from Favourites')}} 
                        </a> 
                    @else 
                ...
                {{-- <a href="#contact_company" class="btn">
