@php
    if(Auth::guard('recruiter')->user()){
        $company=Auth::guard('recruiter')->user()->id;
...
<script type="text/javascript">
    ...
    function eliminarVideo(id) {  
        ...
        $.ajax({
            type: 'GET',
            url: "{{ route('delete.company.video') }}",
            data: {job_id: id},
    ...
    function guardarVideo() {
        ...
        var date = new Date();
        var userid = "{{$company}}";  
        var marcaTiempo = 'C' + userid + 'T' + date.getTime();
        var d2 = d;

        d2 = d2.replace(/[^a-zA-Z0-9]/g, '');
        d2 = d2.replace(' ', '');
        var nameFile = marcaTiempo +'Name'+ d + '.mp4';
        ...
        if (file) {
            ...
            .send(function(err, data) {
                if (data) {
                    ...
                    $('#videoUpload').hide(); 
                    $('#newVideo').hide();  
                    var cambio=$('#videoJobURL').val(nameFile);
                    var idJob=document.getElementsByName('idHidden')[0].value;
                    if(cambio){
                        $.post("{{ route('update.video.url') }}", {nameFile: nameFile,idJob:idJob, _method: 'PUT', _token: '{{ csrf_token() }}'}).done(function(e){
                            mostrarNotificacion();
                            document.getElementById('videoboxTag').innerHTML = `<h5>{{__('Video Job Description')}}</h5>
                <video id="videoJob" width="100%" height="400px" class="video" style="max-height: 400px; background-color: black;" src="https://filescloik.s3.us-east-2.amazonaws.com/videoscompany/${nameFile}" controls type='video/mp4 codecs="avc1.42E01E, mp4a.40.2"'>
                </video>`;
                            // document.getElementById('videoJob').setAttribute('src',`https://filescloik.s3.us-east-2.amazonaws.com/videoscompany/${nameFile}`);
                        }).fail(function(e){
                            console.log(e);
