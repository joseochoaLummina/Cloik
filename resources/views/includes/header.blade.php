                ...
                <div class="navbar navbar-default" role="navigation">
                    <div class="navbar-collapse collapse" id="nav-main">
                        <ul class="nav navbar-nav">
                            ...
                            <li class="{{ Request::url() == route('contact.us.recruiter') ? 'active' : '' }}"><a href="{{ route('contact.us.recruiter') }}">{{__('Contact us')}}</a> </li>
                            <li class="dropdown userbtn"><a href="">{{Auth::guard('recruiter')->user()->printRecruiterImage()}}</a>
                                <ul class="dropdown-menu">
                                    <li><a href="{{route('recruiter.home')}}"><i class="fa fa-tachometer" aria-hidden="true"></i> {{__('Dashboard')}}</a> </li>
                                    <li><a href="{{ route('recruiter.profile') }}"><i class="fa fa-user" aria-hidden="true"></i> {{__('Recruiter Profile')}}</a></li>
                                    @if (Auth::guard('recruiter')->user()->recruiterType())
                                    <li><a href="{{ route('post.job') }}"><i class="fa fa-desktop" aria-hidden="true"></i> {{__('Post a job')}}</a></li>
                                    @endif
                                    {{-- <!-- <li><a href="{{route('recruiter.messages')}}"><i class="fa fa-envelope-o" aria-hidden="true"></i> {{__('Company Messages')}}</a></li> --> --}}
                                    <li><a href="{{ route('recruiter.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-header1').submit();">{{__('Logout')}}</a> </li>
                                    <form id="logout-form-header1" action="{{ route('recruiter.logout') }}" method="POST" style="display: none;">
                                        {{ csrf_field() }}
                                    </form>
                                </ul>
                            </li>
                            @endif @if(!Auth::user() && !Auth::guard('company')->user() && !Auth::guard('recruiter')->user())
