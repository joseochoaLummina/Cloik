...
<div class="modal fade" id="estadisticasEmpresaModal" tabindex="-1" role="dialog" aria-labelledby="estadisticasEmpresaModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="estadisticasEmpresaModalLabel">Company's Statistics</h5>
...
<script>
        ...
        var oTable = $('#companyDatatableAjax').DataTable({
            ...
            ajax: {
                ...
                {data: 'name', name: 'name', render:function(data, type, row){
                    return "<a href='#' onclick='mostrarEstadisticas("+ row.id +");'>" + row.name + "</a>"
                }},
    ...
    function mostrarEstadisticas(id) {
        $('#estadisticasEmpresaModal').modal('show');
        $.post("{{ route('statistics.company') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                    .done(function(response){
                        ...
                        for(let key in response){
                            if(key=="visited Jobs"){
                                ...
                                for(let job of response[key]){
                                    data+=`
                                    <div class="visited-card">
                                        <div>
                                            <div class="visited-card-title"><a href="#" title="${job["title"]}">${job["title"]}</a></div>
                                        </div>

                                        <div class="visited-card-value">
                                            <div>${job["cantidad"]}</div>
                                        </div>
                                    </div>
                                        `
                                    ;
                                ...
                                data+= `
                                    <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12"> 
                                        <a class="dashboard-stat dashboard-stat-v2 blue" href="#">
                                            ...
                                            <div class="details">
                                                <div class="number"> <span data-counter="counterup" data-value="1349">${response[key]}</span> </div>
