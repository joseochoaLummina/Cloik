...
trait ProfileEducationTrait
{
    ...
    public function showProfileEducation(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileEducation)):
            foreach ($user->profileEducation->sortBy('date_completion') as $education):
    ...
    public function showFrontProfileEducation(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileEducation)):
            foreach ($user->profileEducation->sortBy('date_completion') as $education):
                ...
                $dt = $education->getDegreeType('degree_type');
                $dt2 = '';

                if ($dt !== '') {
                    $dt2 = ' - ' . $education->getDegreeType('degree_type') ;
                }
                if ($education->state_id==null) {
                    $state_id=0;
                }else{
                    $state_id=$education->state_id;
                }
                if ($education->city_id==null) {
                    $city_id=0;
                }else{
                ...
                $html .= '<div class="panel panel-info" id="education_' . $education->id . '">
						  <div class="panel-heading"><h4>' . $education->getDegreeLevel('degree_level') . $dt2 . '</h4></div>
						...
						<div class="panel-footer"><a href="javascript:void(0);" onclick="showProfileEducationEditModal(' . $education->id . ',' . $state_id . ',' . $city_id.  ');" class="text text-default">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_education(' . $education->id . ');" class="text text-danger">' . __('Delete') . '</a></div>
						</div>';
            endforeach;
        endif;
    ...
    public function showApplicantProfileEducation(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileEducation)):
            foreach ($user->profileEducation->sortByDesc('id') as $education):
