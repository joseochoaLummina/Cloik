...
trait JobTrait
{
    ...
    public function deleteJob(Request $request)
    {
        $id = $request->input('id');
        try {
            $videoUrl = DB::select('select videoJobURL from jobs where id = :id',['id'=>$id]);
            $job = Job::findOrFail($id);
            JobSkillManager::where('job_id', '=', $id)->delete();
            $job->delete();
            return response()->json([
                'status' => 'ok',
                'videoUrl' => $videoUrl[0]->videoJobURL
            ]);
    ...
    private function assignJobValues($job, $request)
    {
        ...
        $job->description = str_replace("\"", "'", $job->description);
        ...
        $job->confidential = $request->input('confidential');
        if (is_null($request->input('videoJobURL')) || empty($request->input('videoJobURL'))) {
            $job->videoJobURL= "";
        }
        else  {
            $job->videoJobURL = $request->input('videoJobURL');
        }
        $job->video_recommendations=$request->input('recomendations');
