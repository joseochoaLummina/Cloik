...
trait ProfileLanguageTrait
{
    ...
    public function showProfileLanguages(Request $request, $user_id)
    {
        ...
        $profileLanguages=DB::select('select pl.id, l.lang, ll.language_level from profile_languages pl 
        join languages l on pl.language_id=l.id
        join language_levels ll on pl.language_level_id=ll.id where user_id=:id_user order by lang', ["id_user"=>$user_id]);        

        if (isset($user) && count($user->profileLanguages)):
            foreach ($profileLanguages as $language=>$value):
                $html .= '<tr id="language_' . $value->id . '">
						<td><span class="text text-success">' . $value->lang . '</span></td>
						<td><span class="text text-success">' . $value->language_level . '</span></td>
						<td><a href="javascript:;" onclick="showProfileLanguageEditModal(' . $value->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:;" onclick="delete_profile_language(' . $value->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
    ...
    {
        $user = User::find($user_id);
        $languageScore = DB::select('select ll.language_level, llt.score, l.id, l.lang
        from language_levels ll join log_lang_test llt 
        on ll.lang = llt.lang join profile_languages pl
        on pl.language_level_id = ll.language_level_id and pl.user_id = llt.id_user
        join languages l on l.id = pl.language_id
        where llt.is_active = 1 and llt.id_user = :user', ['user'=>$user_id]);
        $html = '<div class="col-mid-12"><table class="table table-bordered table-condensed">';
        // if (isset($user) && count($user->profileLanguages)):
        ...
        if (isset($user) && count($languageScore)):
            foreach ($languageScore as $language):


                $html .= '<tr id="language_' . $language->id . '">
						<td><span class="text text-success">' . $language->lang . '</span></td>
                        <td><span class="text text-success">' . $language->language_level . '</span></td>
                        <td><span class="text text-success">' . round($language->score,0) . '%</span></td></tr>';
            endforeach;
        endif;
        echo $html . '</table></div>';
    }
