...
trait ProfileSkillTrait
{
    ...
    public function showProfileSkills(Request $request, $user_id)
    {
        ...
        // $profileSkills=DB::select('select ps.id, js.job_skill, je.job_experience from profile_skills ps 
		// join job_skills js on ps.job_skill_id=js.id
        // join job_experiences je on ps.job_experience_id=je.id
        // where ps.user_id=:id_user order by job_skill', ['id_user'=>$user_id]);

        $profileSkills=DB::select('select ps.id, js.job_skill from profile_skills ps 
		join job_skills js on ps.job_skill_id=js.id
        where ps.user_id=:id_user order by job_skill', ['id_user'=>$user_id]);

        if (isset($user) && count($user->profileSkills)):
            foreach ($profileSkills as $skill=>$value):
                $html .= '<tr id="skill_' . $value->id . '">
						<td><span class="text text-success">' . $value->job_skill . '</span></td>
						<td><a href="javascript:;" onclick="showProfileSkillEditModal(' . $value->id . ');" class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:;" onclick="delete_profile_skill(' . $value->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
								</tr>';
            endforeach;
        endif;
    ...
    public function storeProfileSkill(ProfileSkillFormRequest $request, $user_id)
    {
        ...
        $profileSkill->job_experience_id = null;
        $profileSkill->save();
        /*         * ************************************ */
