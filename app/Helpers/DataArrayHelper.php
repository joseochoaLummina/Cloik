...
class DataArrayHelper
{
    ...
    public static function onlyIdsStatesArray($country_id)
    {
        $array = State::where('country_id', '=', $country_id)->pluck('state_id')->toArray();
        return $array;
    }
    ...
    {
        $array2 = DB::select('select city, city_id from forge.cities where state_id= :id', ['id'=>$state_id]);
        $array=collect($array2)->pluck('city', 'city_id')->toArray();
        return $array;
    }
    ...
    {
        $array2 = DB::select('select city, city_id from forge.cities where state_id =:id and lang = :lang order by city ASC', ['id'=>$state_id, 'lang' => \App::getLocale()]);
        ...
        if ((int) count($array) === 0) {
            $array2 = DB::select('select city, city_id from forge.cities where state_id =:id and is_default = 1 order by city ASC', ['id'=>$state_id]);
            $array = collect($array2)->pluck('city', 'city_id')->toArray();
        }
