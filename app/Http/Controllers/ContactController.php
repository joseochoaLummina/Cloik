...
class ContactController extends Controller
{
    public function contactCompany()
    {
        if (Auth::user()){
            ...
            if ($array['cumplePerfil'] & $array['cumpleVideo']) {
        ...
        elseif(Auth::guard('company')->check()) {
            $array=ProfileArrayHelper::compruebaCompany();
        
            if ($array ) { $perfil_completo=true; } 
            else { $perfil_completo=false; }
        }else{ $perfil_completo=false; }
        
        $seo = SEO::where('seo.page_title', 'like', 'contact')->first();
        return view('contact.contact_page')
                ->with('seo', $seo)
                ->with('completa',$perfil_completo);
    }
    public function contactRecruiter()
    {
        $seo = SEO::where('seo.page_title', 'like', 'contact')->first();
        return view('contact.contact_page')
                    ->with('seo', $seo); 
