...
<li><a href="javascript:void(0);" onClick="' . $featuredHref . '" id="onclickFeatured' . $companies->id . '"><i class="fa fa-' . $featuredIcon . '" aria-hidden="true"></i>' . $featuredTxt . '</a></li>
    ...
    public function graphics(Request $request){
        ...
        try {
            ...
            $visitedJobs=DB::select('SELECT J.id as job_id, 
                                        J.title as title, 
                                        J.slug as slug, 
                                        J.expiry_date as expiry_date, 
                                        J.created_at as created_at, 
                                        count(JV.id) as cantidad 
                                    FROM job_views JV 
                                    JOIN jobs J ON JV.job_id=J.id 
                                        WHERE J.company_id=:company_id 
                                        GROUP BY J.id 
                                        ORDER BY count(JV.id) desc
                                ;', 
                            ['company_id'=>$company_id]
        );

        $totalJobs=DB::select('SELECT count(1) data
                                        FROM jobs
        ...
        $activeJobs=DB::select('SELECT count(1) data
                                        FROM jobs  
                                        WHERE company_id=:company_id
                                            and is_active=1 
                                    ;',
        ...
        $totalApplicants=DB::select('SELECT count(1) data
                                    FROM job_apply applicants
                                    INNER JOIN  jobs
                                        ON applicants.job_id=jobs.id
                                    WHERE jobs.company_id=:company_id
                                        and applicants.state=1 
                                ;',
                            ['company_id'=>$company_id]
        );
        
        $data=["visited Jobs"=>$visitedJobs,
               "Total Jobs"=>$totalJobs[0]->data,
               "Active Jobs"=>$activeJobs[0]->data,
               "Total Applicants"=>$totalApplicants[0]->data
        ];
