...
class CompaniesController extends Controller
{
    ...
    public function company_listing(Request $request)
    {
        ...
        if($country_id){
            $d1 = Company::where('logo', '!=', null)->where('is_active', 1)->orderBy('name', 'asc')->paginate(20);
            $d2 = Company::where('logo', null)->where('is_active', 1)->orderBy('name', 'asc')->paginate(20);
            $merge = $d1->merge($d2);
            $perPage = 20;
            $currentPage = 1;
            $data['companies'] = new Paginator($merge, $merge->count(), $perPage, $currentPage);
        }else{
            $d1 = Company::where('logo', '!=', null)->where('is_active', 1)->orderBy('name', 'asc')->paginate(20);
            $d2 = Company::where('logo', null)->where('is_active', 1)->orderBy('name', 'asc')->paginate(20);
            $merge = $d1->merge($d2);
    ...
    public function removeFromFavouriteApplicant(Request $request, $application_id, $user_id, $job_id, $company_id)
    {
        ...
        $data['job_id'] = $job_id;
        $data['company_id'] = $company_id;
        $data_save = FavouriteApplicant::where('user_id', $user_id)
                ->where('job_id', '=', $job_id)
