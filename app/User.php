...
class User extends Authenticatable
{
    ...
    {
        $countMNR = DB::select('Select count(*) as msgNotRead FROM messages_center where user_id = :id and state = 0 and receivedfrom = 1;', ['id'=>$this->id]);
        return $countMNR[0]->msgNotRead;
    }

    public function countPendingMessage()
    {
        $countMNR = DB::select('Select count(*) as msgNotRead FROM messages_center where user_id = :id and state = 0 and type = 0 and receivedfrom = 1;', ['id'=>$this->id]);
        return $countMNR[0]->msgNotRead;
    }

    public function countPendingNotification()
    {
        $countMNR = DB::select('Select count(*) as msgNotRead FROM messages_center where user_id = :id and state = 0 and type = 1 and receivedfrom = 1;', ['id'=>$this->id]);
        return $countMNR[0]->msgNotRead;
    }

    public function countPendingInvitation()
    {
        $countMNR = DB::select('Select count(*) as msgNotRead FROM messages_center where user_id = :id and state = 0 and type = 2 and receivedfrom = 1;', ['id'=>$this->id]);
        return $countMNR[0]->msgNotRead;
    }

    public function countPendingAlerts()
    {
        $countMNR = DB::select('Select count(*) as msgNotRead FROM messages_center where user_id = :id and state = 0 and type = 3 and receivedfrom = 1;', ['id'=>$this->id]);
        return $countMNR[0]->msgNotRead;
    }

    public function countPendingMeetings() 
    {
        $countPM = DB::select('select count(*) as pm from meetings where user_id = :id and state = 0 and ( salon IS NULL or salon = "") and current_date() <= planned_date', ['id'=>$this->id]);
        return $countPM[0]->pm;
    }

    public function currentApplications() 
    {
        $countCA = DB::select('select count(*) as ca from job_apply JA inner join jobs J on JA.job_id = J.id
        where JA.user_id = :id and J.is_active = 1 and (timestamp(now()) < J.expiry_date);', ['id'=>$this->id]);
        return $countCA[0]->ca;
    }

    public function countMyVideoCV() 
    {
        $countMVCV = DB::select('select count(*) as vcv from video_apply where id_user = :id;', ['id'=>$this->id]);
        return $countMVCV[0]->vcv;
    }
    ...
    public function isAppliedOnJob($job_id)
    {
        $return = false;
        if (Auth::check()) {
            $count = JobApply::where('user_id', Auth::user()->id)->where('job_id', '=', $job_id)->where('state', '=', 1)->count();
            if ($count > 0)
                $return = true;
        }
        return $return;
